...
## Director continuity and cross-component impact

Before work that can affect the Director or its dependencies, also read `docs/CEVRA_DIRECTOR_DECISIONS.md`. Check impacts in both directions: how the current slice constrains the Director, and how approved Director behavior constrains the current slice. Notify the product owner before implementing a material conflict or silently deferring it; describe the evidence, affected components, user impact, fix-now versus defer costs and recommended bounded path. Pause only affected work when approval is needed. Include a brief `Director impact` assessment in relevant implementation/review closeouts.

Approved decisions remain revisable, not silently disposable. For a material change, obtain the required approval, mark the prior decision SUPERSEDED with date/reason/replacement, and reconcile the Master Context, Director record, applicable ADRs and tests. Use the fix-now versus defer gate in the Director record to minimize total rework without speculative optimization. This policy does not authorize unrelated implementation, merges, publication, deletion or expenditure.
