...
Before work affected by editorial understanding, take selection, duration, pacing or junction planning, read `docs/CEVRA_EDITORIAL_DECISIONS.md` together with the applicable Director record and discussion policy. The eight decisions are approved product direction, not implementation closeout. They describe coordinated planning with shared evidence, not eight independent analysis or mutation systems. Respect the record's pending documentation-integration status until reconciled with the actual repository state.
