...
- **CANONICAL PRODUCT-OWNER RULE — simplest viable solution first:** for every future technical/product doubt, present the most basic reliable solution that makes CEVRA work correctly before a more sophisticated alternative. Complexity is justified only by demonstrated need in quality, reliability, compatibility, security or maintainability. The priority is a functioning product, not architectural sophistication.
...
- **K1 APPROVED — Windows H.264:** use the native Microsoft Media Foundation H.264 path (`h264_mf`) as the first Windows V1 strategy after real smoke/quality validation. Hardware acceleration may be preferred when validated, but NVENC/QSV/AMF are not separate V1 requirements. Do not add OpenH264 unless Media Foundation proves materially inadequate. Do not substitute AV1/VP9 for universal H.264 delivery.
- **K2 APPROVED — HDR→SDR V1:** common HDR source media must work in the normal V1 flow. For SDR output profiles, CEVRA automatically converts supported HDR sources to validated BT.709 SDR; first cross-platform candidate is FFmpeg `zscale`/z.lib + tone mapping. Preserve rich color/VFR/rotation metadata from probe. Advanced HDR/Dolby Vision preservation is post-V1; fail-closed is only the safety fallback when correct conversion is impossible, never silent incorrect color.
- **K3 APPROVED — simple V1 render ownership:** Application coordinates; Composition Engine renders the visual video once; Media Runtime produces/treats final audio and performs the final mux by stream copy when valid. Avoid a large Render Orchestrator and avoid a second lossy visual encode merely to attach audio. More sophisticated frame/pipe transport is deferred unless benchmark evidence proves a material benefit.
- **K4 APPROVED — Composition benchmark additions:** keep the full existing benchmark and add two criteria only: live-preview compatibility in the CEVRA application/WebView path, and whether final rendering can use original/best source media without avoidable lossy intermediates. Preview proxies/approximations remain allowed when final export preserves the original-source path and editorial semantics.
- **K5 APPROVED — V1 platform targets:** official V1 desktop support is macOS Apple Silicon/arm64 + Windows x64. macOS Intel, Linux and Windows ARM are deferred unless near-launch evidence shows an additional target is nearly free and does not delay the primary matrix. Both official targets must pass clean-machine Core Runtime Closure and full core-flow validation.
