...
| K1 | **ACTIVE — Slice 5A FEASIBILITY VERIFIED; PRODUCT ENABLEMENT NOT STARTED.** Native run `36252636826` built the signed FFmpeg 9.0.1 candidate with pinned static zlib 1.3.2, assembled/verified the private runtime, enumerated `h264_mf` and passed real horizontal plus vertical 30000/1001 encode/probe/decode/sync fixtures and their negative control. Production Windows allow-lists remain empty. | Focused independent review, then decide 5B typed enablement: Windows publication identity/lifecycle, capability smoke, Application/Desktop integration and representative hardware. | Binary/runtime feasibility is not Windows export, Desktop or release acceptance; URI/path/mtime alone cannot replace strong publication identity. |
