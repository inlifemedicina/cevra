...
**Versão deste registro:** 1.
**Status:** DIREÇÃO DE PRODUTO APROVADA / IMPLEMENTAÇÃO NÃO AUTORIZADA POR ESTE REGISTRO.
**Decisões aprovadas neste documento:** 14. Não há aprovação implícita de uma decisão 15 ou posterior.
...
Estado remoto conferido antes deste registro:
...
## 3. Handoff e integração documental

Antes de qualquer prompt dependente no CEVRA 3, recuperar o estado real do main e dos PRs. Até a integração, ler explicitamente PR #26 / `docs/editorial-decisions-1-8`: registro editorial v4 para decisões 1–13 e inventário MR; este documento v1 para decisão 14; e o registro do Director no PR #25/revisão aplicável.
...
Resumo para o índice global, a integrar no momento correto:
...
Próximos temas são candidatos à discussão, não aprovações: legendas; composição/layouts e inserções; B-roll/ativos; música/SFX e geração conforme dependências. Continuar a numeração real a partir de 15. Subdividir temas quando necessário para decidir um comportamento por resposta, sem inventar aprovações ou transformar o mapa em escopo automático.
