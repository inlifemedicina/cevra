...
3. refuses to overwrite a pre-existing output;
4. records engine identity/version provenance and invokes the adapter with a stable
   per-attempt `jobId`;
5. verifies that the expected output exists;
...
removed and failed-cleanup paths. Pre-existing files are rejected before engine
execution and are always preserved.
