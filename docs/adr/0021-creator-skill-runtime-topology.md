# ADR 0021 — Creator Skill standalone with shared headless CEVRA Core
...
CEVRA Creator Skill is an agent-native product surface inspired by the direct conversational experience proven by EDVID, while CEVRA Vids Desktop remains the primary independent editor product. The Creator Full must be able to analyze, edit, preview/refine and export a final video without requiring the CEVRA Vids Desktop application to be installed, open or running.

CEVRA already owns stronger audiovisual foundations than the EDVID reference architecture: Project IR and ProjectHistory, managed Media Runtime, transcription/alignment, typed application services, provider boundaries, Creative Intelligence direction, QA and future composition adapters. Duplicating those foundations inside the Creator would create a second editor/runtime stack and divergent audiovisual semantics.

At the same time, making Creator a thin remote control for the Desktop would violate the approved standalone product requirement and make the agent-native product operationally dependent on the desktop UI/application shell.
...
- does **not** require the CEVRA Vids Desktop application, Tauri UI or a running Desktop process;
- may ship its own compatible standalone distribution of the shared CEVRA Core/runtime components;
...
- preserves Project IR as the canonical audiovisual source of truth when Project IR is used;
...
“Headless” is an architectural requirement meaning that reusable audiovisual/application capabilities cannot require the Desktop presentation layer. This ADR does **not** choose the final process topology. The shared Core may later be packaged as libraries plus a local launcher, a dedicated local process/service, or another measured implementation, provided the observable contract above is preserved.
...
Useful public EDVID behavior may be ported according to the existing provenance/license policy. Branding and trade dress remain excluded.
...
A capability may still have a Desktop-specific adapter or presentation layer. Only the reusable domain/application/runtime behavior must remain callable independently.
